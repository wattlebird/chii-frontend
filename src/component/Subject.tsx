...
import {
  Header, Text, Label, Divider, Loader,
...
function Subject() {
  ...
  const { loading: bgmLoading, data: bgmSubjectData } = useGetBangumiSubjectQuery({
    variables: { id: parseInt(id, 10) },
  });
  const history = useHistory();
  const tags = useMemo(() => subjectData?.querySubject?.tags
  ...
  const dateStr = datestr(
    bgmSubjectData?.queryBangumiSubject?.type,
    bgmSubjectData?.queryBangumiSubject?.air_date,
  );
  const rankStr = rankstr(
    bgmSubjectData?.queryBangumiSubject?.type,
    subjectData?.querySubject?.rank,
  );
    ...
    <ArticlePanel>
      <SubjectPanel>
        ...
        <RightPanel>
          ...
          <a href={`https://chii.in/subject/${id}`} rel="noopener noreferrer" target="_blank">
            https://chii.in/subject/
            {id}
          </a>
          {dateStr && <br />}
