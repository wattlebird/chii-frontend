...
function CategoryTable({ category, dataSource, loading }: CategoryTableProps) {
  ...
  const columns = [
    {
      key: 'name', title: '番组动画', dataIndex: 'name', width: 2, render: (__: string, rec: SubjectWithoutTagFragment) => <Link to={`/subject/${rec.id}`}>{parser.parseFromString(rec?.nameCN || rec?.name || '', 'text/html').body.textContent}</Link>,
    },
    {
  ...
  if (typ === 'ANIME') {
    columns.splice(1, 0, {
      key: 'scirank', title: '本站排名', dataIndex: 'sciRank', sorter: (a: SubjectWithoutTagFragment, b: SubjectWithoutTagFragment) => RankSorter(_.get(a, 'siRank'), _.get(b, 'sciRank')),
    });
  }
  return (
    <>
