  ...
  const dataSource = useMemo(() => {
    ...
    if (rankingData?.queryRankingList) {
      dataSource = rankingData.queryRankingList
        ...
        .map<DataSource<SubjectWithoutTagFragment>>(itm => ({...itm as SubjectWithoutTagFragment, key: itm?.id}))
    }
    return dataSource
  }, [rankingData?.queryRankingList, debouncedKeyword])

  const columns = [
    {key: "name", title: "番组动画", dataIndex: "name", width: 2, render: (_: any, rec: SubjectWithoutTagFragment) => <Link to={`/subject/${rec.id}`}>
    ...
    {key: "date", title: "日期", dataIndex: "date", render: (val: string) => {
      if (val.startsWith("0001")) return null;
      else return (new Date(val)).toLocaleDateString("zh-CN");
