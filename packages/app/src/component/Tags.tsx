...
function Tags() {
  ...
  useEffect(() => {
    ...
    const queryTags = qs
      ? qs
          ...
          .filter((x) => !!x)
      : []
    if (queryTags.length !== tags.length || queryTags.some((t, i) => t !== tags[i])) {
      setTags(queryTags)
    }
  ...
  const onSelect = (_: any, { value }: any) => {
    ...
    navigate(`/tags?q=${encodeURI(value.join(' '))}`)
  }

  const appendSearchTag = (tag: string) => {
    const newtags = [...tags, tag]
    navigate(`/tags?q=${encodeURI(newtags.join(' '))}`)
  }

  return (
    <ArticlePanel column gap='gap.small'>
      <TitlePanel>
