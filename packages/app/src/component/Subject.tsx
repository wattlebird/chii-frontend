      ...
      subjectData?.querySubject?.tags
        ...
        .map((itm) => (
          <Label
            ...
            content={
              <span>
                {itm?.tag} <small>{itm?.tagCount}</small>
              </span>
            }
            styles={{ margin: '0.25rem', cursor: 'pointer' }}
            onClick={() => {
              if (itm?.tag) navigate(`/tags?q=${encodeURI(itm?.tag)}`)
            }}
          />
        )),
    [subjectData?.querySubject?.tags]
    ...
    <ArticlePanel>
      <SubjectPanel>
        ...
        <RightPanel>
          <img
            width='320'
            src={bgmSubjectData?.queryBangumiSubject?.images?.large}
            alt={subjectData?.querySubject?.name}
          />
          <br />
