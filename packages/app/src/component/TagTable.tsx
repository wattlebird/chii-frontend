...
function CategoryTable({ category, dataSource, loading }: CategoryTableProps) {
  ...
  const categorySwitcher = Object.keys(category).map((itm) => (
    <Pill
      appearance={typ === itm ? 'outline' : 'filled'}
      content={`${catNameLut[itm as SubjectType]} ${category[itm as SubjectType]}`}
      onClick={() => setTyp(itm)}
    />
  ))
  ...
  const columns = [
    {
      ...
      dataIndex: 'name',
      width: 2,
      render: (__: string, rec: SubjectWithoutTagFragment) => (
    ...
    },
    {
      ...
      title: 'Bangumi 排名',
      dataIndex: 'rank',
      sorter: (a: SubjectWithoutTagFragment, b: SubjectWithoutTagFragment) =>
        RankSorter(_.get(a, 'rank'), _.get(b, 'rank'))
    }
  ]
  if (typ === 'ANIME') {
    columns.splice(1, 0, {
      ...
      dataIndex: 'sciRank',
      sorter: (a: SubjectWithoutTagFragment, b: SubjectWithoutTagFragment) =>
        RankSorter(_.get(a, 'siRank'), _.get(b, 'sciRank'))
    })
  }
  return (
    <>
...
function TagTable({ tags }: TagTableProps) {
  ...
  const dataSource = useMemo<DataSource<SubjectWithoutTagFragment>[]>(() => {
    let rst: DataSource<SubjectWithoutTagFragment>[] = []
    if (data?.searchByTag) {
      rst = data.searchByTag.map((itm) => ({ ...itm, key: itm?.id }))
    }
    return rst
  }, [data?.searchByTag])
  ...
  if (error) {
    ...
    return <Alert danger content="Can't load the search result" />
  }
